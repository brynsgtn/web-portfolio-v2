...
const TimelineItem = ({ children, icon: Icon, isLeft }) => {
    ...
    return (
        <li ref={itemRef}>
            ...
            <div className={`${isLeft ? 'timeline-start lg:text-start' : 'timeline-end  text-end'} mb-10 w-full transition-all duration-700 ${isVisible
...
const ProfessionalBackground = () => {
    ...
    // Combine all timeline items with type and sort date
    const allItems = [
        ...education.map(edu => ({
            type: 'education',
            icon: GraduationCap,
            date: edu.year.split('-')[0], // Get start year for sorting
            displayDate: edu.year,
            title: edu.school,
            subtitle: edu.course,
            data: edu
        })),
        ...experience.map(exp => ({
            type: 'experience',
            icon: Briefcase,
            date: exp.duration.split('-')[1].trim(), // Get end year for sorting
            displayDate: exp.duration,
            title: exp.position,
        ...
        // Convert dates to comparable format for sorting (latest first)
        ...
        <div className="min-h-screen py-20 xl:px-50">
            ...
            <div className="flex gap-2 flex-wrap mt-10 mb-20">
                <button
                    ...
                    className={`px-4 py-2 rounded-lg transition-all flex items-center gap-2 hover:cursor-pointer transform hover:scale-110 hover:text-base-100 hover:bg-success ${filter === 'all'
                ...
                <button
                    ...
                    className={`px-4 py-2 rounded-lg transition-all flex items-center gap-2 hover:cursor-pointer transform hover:scale-110 hover:text-base-100 hover:bg-success ${filter === 'certification'
                        ? 'bg-success text-base-100'
                        : 'bg-base-200 text-primary-content hover:bg-base-300'
                        }`}
                >
                ...
                <button
                    ...
                    className={`px-4 py-2 rounded-lg transition-all flex items-center gap-2 hover:cursor-pointer transform hover:scale-110 hover:text-base-100 hover:bg-success ${filter === 'education'
                        ? 'bg-success text-base-100'
                        : 'bg-base-200 text-primary-content hover:bg-base-300'
                        }`}
                >
                ...
                <button
                    ...
                    className={`px-4 py-2 rounded-lg transition-all flex items-center gap-2 hover:cursor-pointer transform hover:scale-110 hover:text-base-100 hover:bg-success ${filter === 'experience'
                        ? 'bg-success text-base-100'
                        : 'bg-base-200 text-primary-content hover:bg-base-300'
                        }`}
                >
            ...
            <ul className="timeline timeline-snap-icon max-w-6xl mx-auto timeline-vertical">
                ...
                {filteredItems.map((item, i) => (
                    ...
                    >
                        ...
                        {/* Certificate link only if cert */}
                        {item.type === 'certification' && (
                            <a
                                href={item.data.link}
                                target="_blank"
                                rel="noopener noreferrer"
                                className="inline-flex items-center gap-2 text-success hover:text-primary-content transition-colors group mt-2 block"
                            >
