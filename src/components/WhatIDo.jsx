...
const AccordionItem = ({ item, isOpen, onToggle, trigger }) => {
  return (
    <motion.div
        ...
        <div className="px-6 pb-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
            {item.technologies.map((tech, index) => (
              <div key={index} className="flex items-center gap-2 text-secondary-content">
...
const WhatIDo = () => {
  ...
  const handleToggle = (id) => setOpenAccordion(openAccordion === id ? null : id);

  const [direction, setDirection] = useState("right");
  useEffect(() => {
    const media = window.matchMedia("(min-width: 1024px)");
    const update = () => setDirection(media.matches ? "up" : "right");
    update();
  ...
  const sectionInView = useInView(sectionRef, { once: false, margin: "-150px 0px -150px 0px" });

  return (
    <div ref={sectionRef} className="min-h-screen pt-20 xl:px-50">
      ...
      <div className="lg:flex justify-around items-center">
        ...
        >
          <StarBorder
            className="backdrop-blur-sm max-w-4xl 2xl:w-500 mx-auto shadow-2xl"
            color="cyan"
            speed="5s"
          >
        ...
        <motion.div
          ...
          className="px-10 mt-10 h-20 lg:h-130 align-middle border border-success rounded-3xl shadow-2xl"
        >
          <LogoLoop
            logos={techLogos}
            speed={30}
